/**
 * contrast-audit.js — Audit color contrast against WCAG AA/AAA and write a
 * designed report page back into the Figma file.
 * ---------------------------------------------------------------------------
 * This is the canonical implementation. It runs unchanged in two places:
 *
 *   1. Via the Figma MCP `use_figma` tool (load the `figma-use` skill first).
 *      Paste this whole file as the script body; the tail invokes the function
 *      and returns a summary object.
 *
 *   2. As a standalone Figma plugin — see ./standalone-plugin/, which wraps the
 *      exact same `runContrastAudit` function and calls figma.closePlugin().
 *
 * WHAT IT DOES
 *   For every top-level frame on the CURRENT page it measures:
 *     • Text contrast — each run of text vs the background behind it, scored
 *       against WCAG 1.4.3 (AA) and 1.4.6 (AAA), with the large-text exception.
 *     • Non-text/UI contrast — visible solid strokes (input borders, dividers,
 *       button outlines, icon strokes) vs their background, scored against the
 *       WCAG 1.4.11 threshold of 3:1. This is a *heuristic* flag for manual
 *       review, not a definitive pass — see references/wcag-thresholds.md.
 *   Then it creates a NEW page and draws a clean report: a legend explaining the
 *   thresholds and one row per audited frame with PASS/FAIL badges. Each frame's
 *   name is a clickable link that jumps straight to that frame, and its node id
 *   is printed beneath so the address is always visible but never cluttered.
 *
 * IT NEVER changes the designs it audits. It only READS them and CREATES a new
 * report page.
 *
 * Plugin API used (all verified against the official typings — nothing invented):
 *   - figma.currentPage / .children / .name
 *   - figma.skipInvisibleInstanceChildren        (perf: skip hidden instance internals)
 *   - figma.createPage()                          (Figma Design only)
 *   - figma.setCurrentPageAsync(page)             (switch to the report when done)
 *   - figma.listAvailableFontsAsync() / figma.loadFontAsync({family, style})
 *   - figma.createFrame() / figma.createText()
 *   - node.findAll(callback)                      (collect text + UI nodes)
 *   - node.getStyledTextSegments([...])           (per-run color, size, weight — no "mixed")
 *   - node.fills / node.strokes / node.opacity / node.visible
 *   - node.absoluteBoundingBox                    (rough z-order/overlap heuristics)
 *   - auto-layout props (layoutMode, itemSpacing, padding*, primary/counterAxis*)
 *   - node.layoutSizingHorizontal/Vertical = 'FILL' (set AFTER appendChild)
 *   - node.strokeBottomWeight / strokeAlign       (row dividers)
 *   - TextNode.setRangeHyperlink(start, end, {type:'NODE', value:nodeId})
 *       NOTE: a NODE hyperlink target must be a page or a frame — our targets are
 *       top-level frames, so this is valid.
 *   - figma.commitUndo()                          (one undo step for the report)
 * ---------------------------------------------------------------------------
 */
...
// --- invocation (for use_figma) --------------------------------------------
// When run via the Figma MCP `use_figma` tool, this returns the summary to the
// caller. Adjust the options object to taste.
return await runContrastAudit({
  includeNonText: true,
  switchToReport: true,
});
